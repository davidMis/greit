...
def flatten_K(N, K, adj):
    ...
    for i in range(adj.shape[0]):
        for j in range(i + 1, adj.shape[1]):
            if adj[i, j] == 1:
                flat_K = flat_K.at[k, 0].set(K[i, j])
                k = k + 1
    return flat_K
...
#     u = jnp.zeros([num_edges(N), K_dim(N)])
#     c = jnp.zeros([num_edges(N), K_dim(N), K_dim(N)])
...
#             y = y.at[i].set(1.0)
#             psi = Lambda @ y
#             x_opp = jnp.linalg.solve(Lambda[zero_c_edge, opp_c_edge], -psi[zero_c_edge])
#             x = jnp.zeros(4*N)
#             x = x.at[opp_c_edge].set(x_opp)
#             x = x.at[i].set(1.0)
#             phi = Lambda @ x
...
#                 c = c.at[i,b_idx,i_idx].set(phi[b_idx])
...
#         K = K.at[q,z].set(Lambda[q,W_idx] @ jnp.linalg.solve(Lambda[E_idx, W_idx], Lambda[E_idx, q]) - Lambda[q,q])
#     # East
#     for i, q in enumerate(range(1*N, 2*N)):
#         z = interior_index(N,1+i,N)  # interior node
#         K = K.at[q,z].set(Lambda[q,S_idx] @ jnp.linalg.solve(Lambda[N_idx, S_idx], Lambda[N_idx, q]) - Lambda[q,q])
#     # North
#     for i, q in enumerate(range(2*N, 3*N)):
#         z = interior_index(N,N,N-i)  # interior node
#         K = K.at[q,z].set(Lambda[q,E_idx] @ jnp.linalg.solve(Lambda[W_idx, E_idx], Lambda[W_idx, q]) - Lambda[q,q])
#     # West
#     for i, q in enumerate(range(3*N, 4*N)):
#         z = interior_index(N,N-i,1)  # interior node
#         K = K.at[q,z].set(Lambda[q,N_idx] @ jnp.linalg.solve(Lambda[S_idx, N_idx], Lambda[S_idx, q]) - Lambda[q,q])

#     # K = K.at[q1,r].set(Lambda[q1,E_idx] @ jnp.linalg.solve(Lambda[W_idx, E_idx], Lambda[W_idx, q1]) - Lambda[q1,q1])
#     # K = K.at[q2,s1].set(Lambda[q2,E_idx] @ jnp.linalg.solve(Lambda[W_idx, E_idx], Lambda[W_idx, q2]) - Lambda[q2,q2])
#     # K = K.at[p1,r].set(Lambda[p1,S_idx] @ jnp.linalg.solve(Lambda[N_idx, S_idx], Lambda[N_idx, p1]) - Lambda[p1,p1])
...
#         u = u.at[q2,r1].set(-c[q2,q1,r1]/K[q1,r1])
#         K = K.at[r2,r1].set( c[q2,q2,r2] / -u[q2,r1] )
...
#     if N==2:
#         K = K + K.T # Make Symmetric
#         K = K - jnp.diag(jnp.sum(K, axis=1)) # Fill in diagonal entries
#         return K
...
#         u = u.at[q3,r1].set( -c[q3,q1,r1] / K[q1,r1] )
#         c = c.at[q3,s1,r1].set( K[s1,r1]*(u[q3,s1]-u[q3,r1]) ) # Ohm's Law
#         c = c.at[q3,t1,s1].set( -c[q3,q3,t1] )
#         c = c.at[q3,t2,s1].set( -c[q3,q2,s1] + c[q3,t1,s1] + c[q3,s1,r1] )
#         K = K.at[t2,s1].set( -c[q3,t2,s1] / u[q3,s1])
